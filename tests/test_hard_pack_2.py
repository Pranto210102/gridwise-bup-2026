...
def hard_pack_2_cases():
    with open("hard_test_pack_2.json", "r", encoding="utf-8") as f:
        data = json.load(f)
    return data["cases"]


def test_hard_pack_2_cases(hard_pack_2_cases):
    """
    Validates 10 brand-new production-level hard cases (HARD-11 to HARD-20):
    - Exact semantic directive interpretation
    - Flow balance at each hour (LHS == RHS)
    - Full battery neutrality at hour 23
    - Lockout and reserve compliance
    """
    for case in hard_pack_2_cases:
        cid = case["id"]
        inp = case["input"]
        ...
        for idx, (act, exp) in enumerate(zip(actual_directives, exp_directives)):
            ...
            if not exp["applies"]:
                ...
                assert act_adj.get("hours") == exp_adj.get("hours"), f"{cid} Note {idx} hours mismatch"
        ...
        # 2. Hourly Plan Physical Verification
        ...
        for h_entry in plan:
            ...
            # Flow balance: demand + battery_charge = grid + solar_used + battery_discharge
            lhs = dem + bc
            rhs = g + s + bd
            assert abs(lhs - rhs) < 0.05, f"{cid} Hour {h}: imbalance LHS {lhs} != RHS {rhs}"

            # Limits
            assert s <= effective_solar[h] + 1e-2
            assert bc <= battery_cfg["max_charge_kwh_per_hour"] + 1e-2
            assert bd <= battery_cfg["max_discharge_kwh_per_hour"] + 1e-2

            if charge_blocked[h]:
                assert bc < 1e-2, f"{cid} Hour {h}: charge blocked but charged {bc}"
            if discharge_blocked[h]:
                assert bd < 1e-2, f"{cid} Hour {h}: discharge blocked but discharged {bd}"
            if grid_caps[h] < float("inf") and (dem <= grid_caps[h] + effective_solar[h] + battery_cfg["max_discharge_kwh_per_hour"] + 1e-2):
                assert g <= grid_caps[h] + 1e-2

            # State continuity
            next_energy = curr_energy + bc - bd
            assert abs(be - next_energy) < 0.05
            assert be <= battery_cfg["capacity_kwh"] + 1e-2
            assert be >= hourly_min_reserve[h] - 1e-2
        ...
        # Neutrality
        assert abs(curr_energy - battery_cfg["initial_energy_kwh"]) < 0.1, f"{cid}: Neutrality violated"
